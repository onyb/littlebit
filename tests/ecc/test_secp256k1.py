...
def test_sec_deserialization():
    ...
    for coefficient, uncompressed, compressed in inputs:
        ...
        assert S256Point.parse(bytes.fromhex(uncompressed)) == coefficient * G
        assert S256Point.parse(bytes.fromhex(compressed)) == coefficient * G
